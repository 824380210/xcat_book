...
```
Note:
    "ssh nodexx ipmitool chassis  identify 15" command will go with the 10G network channel ,so it will related with the 10G network ,you need to make sure the LED is turn on by sequence to make sure all cable is correct connect 
